...
PCA가 다중 클러스터를 무시하게 되는 경위는 이렇다. 서로 다른 분포가 여러 개 섞여 있어도 PCA는 이들을 하나의 분산으로 놓고 보기 때문에, 그 전체에서 분산이 가장 큰 축부터 차례로 찾아낸다. LDA 역시 클래스 경계를 직선으로만 그을 수 있어, 경계가 비선형이면 축을 어떻게 잡아도 두 클래스가 갈라지지 않는다.
...
네 가지 한계 중에서도 위상 보존 실패가 가장 결정적인 문제다. 앞의 세 가지가 구조를 흐리게 만드는 문제라면, 위상의 왜곡은 없던 덩어리가 만들어지고 있던 고리가 끊어지며 원래 없던 연결이 새로 생기는 문제다. 투영을 거치는 동안 형상 자체가 다른 것으로 바뀌기 때문에, 위상 보존 실패는 선형 투영이 가진 문제 가운데 가장 큰 것으로 꼽힌다.
...
MDS 자체는 선형 방법론이다. 그런데 거리를 재는 전처리 자리에 비선형이 될 수 있는 그래프 기반 거리 함수를 끼워 넣으면서 Isomap이라는 비선형 차원 축소 방법이 만들어졌다. Isomap을 쓰는 시점에서 이미 비선형 차원 축소가 시작된 셈이다. 여기서부터 t-SNE와 UMAP으로 대표되는 비선형 차원 축소 방법으로 이어진다.
...
t-SNE의 저차원 배치에서 클래스가 뚜렷하게 갈라져 보이는 이유는 두 분포의 꼬리 차이에 있다. 고차원에서는 정규분포로, 저차원에서는 꼬리가 두꺼운 t분포로 같은 확률을 맞추면 가까운 점은 그대로 남고 조금 떨어진 점은 정규분포를 쓸 때보다 더 멀리 밀려난다. 그래서 저차원에 매핑된 결과에서 클래스 사이의 형태가 더 잘 분리되어 보인다.

LLE는 각 점의 위치를 이웃들의 가중합으로 계산하므로 주변 점들과의 관계가 아주 정교하게 표현된다. 그 대신 계산에 들어오는 것이 근처의 점들뿐이어서, 국소적인 특징은 잘 잡아내지만 전체적인 형태는 잘 보이지 않는다.
...
네 기법의 MNIST 투영 결과는 3차원 좌표로도 함께 저장해 두었다. 실습 자료에 들어 있는 html 파일을 내려받아 브라우저에서 열면 시점을 돌려 가며 덩어리들이 어떻게 놓였는지 직접 확인할 수 있다. 같은 자료에 코드도 함께 들어 있어 그림을 직접 다시 만들어 볼 수 있다. 
...
이 정리는 어느 정도의 표본을 손에 쥐었을 때 그 거리 관계를 온전히 지키려면 최소한 몇 차원이 필요한지를 간단히 가늠해 보는 도구로 쓴다. 표본 수보다 허용 왜곡 쪽에 훨씬 민감하다는 성질이 그 판단의 축이 된다. 지금까지 본 투영 방법들은 무엇을 살릴지 고르는 형태의 알고리즘이어서, 하나를 고르면 다른 하나는 포기하는 구조가 된다.
...
네 경우를 관통하는 결론은 하나다. 목적이 다르면 잘된 투영의 정의도 달라지므로, 기법을 고르는 일은 목적을 정한 다음에 온다. 전처리로 쓸 때처럼 클러스터가 눈에 보이는지가 아무 상관이 없는 경우도 있어서, 시각적으로 선명한 것과 잘된 투영은 서로 별개일 수 있다. 평가 기준 자체를 목적의 함수로 두어야 한다는 말이 이 뜻이다.

이상 탐지에서는 이 성질을 거꾸로 쓴다. 위상 관점으로 보면 노이즈와 이상치는 고립된 형태로 남는데, 그렇게 고립된 것이 얼마나 있는지 세고 잘라 내는 방식으로 이상치를 분리해 낼 수 있다. 이 자리에서 잘된 투영은 정상과 이상을 잘 갈라 놓는 투영이고, 위상을 함께 보는 일이 그만큼 중요해진다.
...
### 해석 가능성과 설계 가능성 — 딥러닝이 바꾸는 자리

표의 앞 두 항목은 되돌리는 문제와 새로 들어오는 데이터의 문제다. 딥러닝에서는 오토인코더의 디코더가 역변환을 맡아, 투영된 값을 다시 넣어 원래 형태로 되돌린다. 반면 t-SNE나 UMAP으로 본다는 것은 지금까지 모아 둔 데이터를 한 번에 처리한다는 뜻이어서, 새로 들어온 표본을 같은 공간에 놓을 방법이 없다.

설명 불가능성은 축의 문제다. 세 개의 축으로 투영해 놓고 나면 어떤 점의 한 축 값이 크다는 것이 원래 공간에서 무엇을 뜻하는지 알 수 없다. 투영해서 구조를 보는 것까지는 되지만 투영된 축의 성격을 설명할 수는 없다. 딥러닝이 만들어 내는 투영 결과를 Disentangled Representation이라고 부르는 것은 이 지점을 두고 하는 말이다. 다만 풀렸다는 것은 눈에 보인다는 뜻이 아니라 해석할 수 있다는 뜻이다.

전역 구조와 국소 구조를 함께 살리는 일도 기본적인 비선형 투영에서는 되지 않고 둘 중 한쪽으로 기울어진다. 딥러닝에서는 손실 함수를 어떻게 설계하느냐에 따라 두 구조를 함께 통제할 수 있어서, 없던 설계의 여지가 생긴다. 대신 치르는 값이 있다. 계산 비용이 높고, 더 결정적으로는 데이터가 많아야 한다. 데이터가 적으면 딥러닝을 쓸 수 없고 전통적인 머신러닝을 쓸 수밖에 없다.
