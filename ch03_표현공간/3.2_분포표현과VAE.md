<!-- ───────────────── 강의 음성 기반 보완 제안 ─────────────────
  이 파일의 제안 11건 (개념보강 2 / 그림해설 8 / 실무맥락 1)
...
<!-- +강의 id=3.2_분포표현과VAE-01 w=w5 ts=[21:38] [22:26] 유형=그림해설 채택=? -->
두 그림 모두 분포 전체가 한쪽으로 찌그러져 있어 좌우도 위아래도 대칭을 이루지 못한다. 이런 공간에서 격자 간격으로 좌표를 뽑으면 어떤 자리에는 점이 몰려 있고 어떤 자리는 텅 비므로, 클래스마다 샘플링 간격을 달리 잡아야 하는데 그것이 쉽지 않다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-02 w=w5 ts=[24:43] [25:30] 유형=그림해설 채택=? -->
잠재공간이 2차원이라면 인코더는 두 축 각각에 대해 평균과 분산을 하나씩, 모두 네 개의 값을 내놓는다. 가운데 칸이 분포로 바뀌었으므로 같은 이미지를 넣어도 샘플링되는 $z$ 는 매번 달라지고, 디코더가 받는 값도 그때마다 조금씩 바뀐다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-03 w=w5 ts=[27:50] [28:36] 유형=그림해설 채택=? -->
점 표현에서는 웃음 0.99, 피부톤 0.85, 성별 -0.73처럼 축마다 값 하나를 단정해서 적어야 한다. 분포 표현에서는 확신이 큰 성별 축의 분포가 좁게 나타나고, 사진 한 장으로는 판단하기 어려운 피부톤 축의 분포는 한쪽으로 넓게 퍼진다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-04 w=w5 ts=[42:04] [42:52] 유형=그림해설 채택=? -->
제약이 없는 쪽 그림은 중심이 한곳으로 모이지 못하고 클래스마다 폭이 크게 달라진다. KL 손실로 학습하면 중심이 일정해지고 분포 크기의 편차가 줄어 서로 비슷한 크기로 가까이 놓인다. 지나치게 촘촘해 들여다보기 어렵던 영역도 함께 정리된다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-05 w=w5 ts=[40:29] 유형=개념보강 채택=? -->
두 성질이 함께 성립하는 근거는 서로 다르다. 벡터가 주변에 함께 나타나는 비슷한 의미의 단어들의 분포에 기반해 만들어진다는 점에서 분포형이고, 그렇게 얻은 정보가 다차원 벡터의 여러 성분에 나뉘어 담긴다는 점에서 분산형이다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-06 w=w5 ts=[45:13] [46:00] 유형=그림해설 채택=? -->
이 그림의 데이터셋은 같은 사람의 얼굴을 좌에서 우로 조금씩 돌린 사진과 웃는 얼굴부터 화난 얼굴까지의 사진으로 이루어져 있다. 한 축의 값을 고정한 채 다른 축만 훑으면 머리 방향은 그대로인 상태에서 표정만 바뀌는 결과를 얻는다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-07 w=w5 ts=[49:57] 유형=그림해설 채택=? -->
이 그림은 분산을 그대로 두고 평균만 좌우로 옮겨 가며 가능도가 어떻게 변하는지를 보여준다. 평균이 데이터 쪽으로 다가가면 가능도가 가파르게 올라갔다가 지나치면 다시 떨어진다. 그 정점, 곧 미분값이 0이 되는 지점을 찾는 것이 최대가능도추정이다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-08 w=w5 ts=[51:32] [52:17] 유형=그림해설 채택=? -->
커튼 뒤로 누군가 지나가 실루엣만 보였을 때 그것이 철수인지 영희인지 가리는 일이 사후확률이다. 지금이 오후 한 시이고 철수는 초등학생, 영희는 유치원생이라면 철수가 그 시각에 집에 있을 선험적 확률은 낮다. 이 값이 사전확률로 들어간다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-09 w=w5 ts=[80:27] [80:57] 유형=개념보강 채택=? -->
세 이름 가운데 marginal 을 옮긴 '주변'은 우리말 뜻과 잘 맞지 않는다. 초창기에 marginal 을 주변으로, marginalize 를 주변화로 번역한 관행이 그대로 굳어진 결과이고, 그래서 용어의 어감과 실제 연산이 어긋난 채로 통용된다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-10 w=w5 ts=[100:11] [100:58] 유형=그림해설 채택=? -->
$\beta = 250$ 쪽에서는 한 잠재 변수를 순회시키면 얼굴의 머리 방향만 차례로 바뀐다. $\beta = 1$ 쪽에서는 같은 조작에도 머리 방향의 변화가 뚜렷하지 않고 다른 특징까지 함께 흔들린다. 특징이 얽혀 있어 어떤 변수가 무엇을 담당하는지 해석하기 어렵다는 것이 이 비교의 요지다.
<!-- /+강의 -->
...
<!-- +강의 id=3.2_분포표현과VAE-11 w=w5 ts=[19:12] [107:17] 유형=실무맥락 채택=? -->
VAE에 노이즈를 넣는 것은 결국 데이터를 증강하는 것과 같은 조작이다. 증강은 과적합을 줄이기 위해 가장 먼저 시도하는 방법이므로, Denoising VAE도 특별한 기법이라기보다 늘 쓰던 관행을 VAE에 그대로 적용한 것으로 보면 된다.
<!-- /+강의 -->
