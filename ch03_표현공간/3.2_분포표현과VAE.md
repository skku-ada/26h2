...
<!-- 원문 수식 확인 필요: 원문의 "P(x_i,θ)=x_i P(θ)" 는 OMML 변환 과정에서 깨진 것으로 보인다. 의미는 "데이터 x_i 가 모수 θ의 분포에서 갖는 밀도값 p(x_i | θ)" 이다. -->
...
<!-- 원문 수식 확인 필요: 원문 중간 전개는 "(σ_1^2 + μ_1 − μ_2)^2 / σ_2^2" 로 되어 있으나, 마지막 줄의 결과와 정합하도록 "σ_1^2 + (μ_1 − μ_2)^2" 를 2σ_2^2 로 나눈 형태로 정정하여 옮겼다. -->
