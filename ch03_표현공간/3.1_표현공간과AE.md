...
두 접근의 차이가 한눈에 드러나지 않는 이유가 있다. 전통적인 기법도 새로운 좌표계를 만들어 그곳에 데이터를 매핑하므로 겉으로는 딥러닝과 비슷해 보인다. 갈라지는 지점은 좌표계를 만드느냐가 아니라, 데이터의 특성을 독립적으로 매핑하느냐에 있다.

여기서 투영되는 위치의 성격도 달라진다. 전통적인 기법에서 새 데이터가 얻는 것은 기존 데이터가 만든 좌표계 위의 상대적 위치이지만, 학습된 표현 함수에서는 자기 자신만으로 계산된 위치에 놓인다. 상대적 위치로 얻은 결과는 해석이 쉽지 않고, 기대한 대로 되지 않는 경우도 흔하다.
...
통신 비유를 조금 더 밀고 가면 AE가 놓인 자리가 분명해진다. 생각을 문장으로 인코딩하는 과정은 품질이 완전하지 않아서, 듣는 쪽이 디코딩한 결과가 원래의 생각과 상당히 달라진다. AE도 같은 조건에 있다. 압축된 신호만 남긴 뒤 그것으로 원본을 되살리는 구조이므로, AE가 목표로 삼는 것은 완전한 복제가 아니라 입력과 유사한 재구성이다.

학습 신호는 밖에서 오지 않는다. 정답 레이블 대신 입력 데이터 자신을 목표로 삼기 때문에, AE의 학습을 self-supervised learning이라 부르기도 한다.

압축은 구조에서 따라 나온다. Encoder는 큰 입력 데이터를 더 작은 크기의 층에 매핑해야 하고, 그 작은 노드에 담으려면 데이터를 압축하게 된다. 그리고 decoder는 그렇게 남은 고정 벡터만 가지고 다시 입력 데이터를 만들어 내야 한다. 압축된 표현을 찾기 위해 데이터를 압축하고, 그 압축된 정보만으로 입력을 재구성하는 것이 AE의 구조다.
...
이 세 가지 구조는 각각 다른 방식으로 확인한다. 잠재 공간을 시각화해 밀집 영역이 어떻게 나뉘는지 보고, 연속적으로 변화하는 저차원 구조가 있는지 확인하며, 특정 축을 따라가며 그 축이 어떤 의미를 갖는지 해석한다.

용어를 읽을 때 한 가지를 전제해 두면 편하다. 여기서 공간과 좌표계는 같은 뜻으로 쓴다. 데이터가 변환을 거쳐 다시 기술되는 좌표계 자체가 표현 공간이므로, 관측 공간과 표현 공간의 구분도 결국 좌표계의 구분이다.
...
이 용어들이 섞여 쓰이는 것은 정의가 같아서가 아니다. 정의는 조금씩 다르고, 구조의 어느 부분을 강조하느냐에 따라 다른 이름이 선택된다. 그래서 문헌을 읽을 때는 같은 대상을 가리키는 여러 이름 사이에서 무엇이 달라지는지를 그때그때 들여다볼 필요가 있다.
...
오른쪽 t-SNE 그림은 encoder가 만든 latent code를 다시 임베딩해 본 결과다. MNIST를 곧바로 t-SNE로 표현한 것보다, 구조화된 code를 받아 표현한 쪽이 클래스별로 훨씬 잘 뭉친다. AE가 재구성한 좌표계가 이미 구조를 드러내고 있다는 뜻이다.
### 압축과 복원을 함께 학습한다
...
선형 AE와 PCA가 겹치는 지점은 앞서 PCA를 다룰 때 이미 나왔다. 하나의 뉴런이 PCA의 한 성분에 대응한다는 설명이 그것이다. 활성화 함수만 빼면 둘은 기능적으로 거의 같아지며, 비선형 활성화 함수를 두는 것이 더 복잡한 구조까지 표현하게 만드는 차이다.
...
Latent space가 manifold의 구조를 기술하는 좌표계라는 말에는, 둘이 일대일로 대응하지는 않는다는 뜻도 함께 들어 있다. 좌표계는 manifold를 펼치듯 기술하려 할 뿐이고, 학습이 복원 손실을 줄이는 방향으로만 진행되므로 원래 구조와 어긋나는 부분이 남는다.
...
CE가 재는 것은 입력 데이터의 분포와 디코딩된 데이터의 분포 사이의 cross-entropy다. 재구성 오차는 데이터의 성격과 목적에 따라 여러 형태로 만들어질 수 있고, 어느 것을 고르느냐에 따라 손실 함수가 민감하게 반응하는 지점이 달라진다.
...
두 지표는 이 절에서 새로 만든 것이 아니다. Silhouette Score는 앞서 군집 분석에서 쓰던 지표를 그대로 가져온 것이며, 학습으로 만들어진 잠재 공간에도 그대로 적용된다. 잠재 공간이 잘 학습되었는지를 눈이 아니라 수치로 답하기 위한 도구다.
...
다만 일반적인 AE의 잠재 공간이 구면인 경우는 흔하지 않다. 구면 보간이 필요한 상황은 벡터가 구의 내부가 아니라 표면에만 분포할 때이며, hyperspherical하다는 말이 가리키는 것이 그 상태다.
...
여기서 dense하다는 것은 잠재 공간에 정보가 밀집해 있다는 뜻이고, sparse하면 중간에 구멍이 뚫려 있다는 뜻이다. 세 번째 행의 도약은 클러스터가 잘 분리되어 그 사이에 빈 영역이 생긴 결과일 수도 있어, 해석하기 나름인 부분이 남는다.
...
정보이론으로 옮겨 보는 대상은 AE의 정보 흐름 전체다. 입력 데이터가 encoder를 거쳐 latent code가 되고, decoder가 그것을 받아 복원하는 흐름 자체가 AE이며, 이 흐름의 각 단계에서 정보가 얼마나 남는지를 하나의 척도로 재는 것이 여기서의 관점이다.
...
이 부등식이 성립하는 이유는 변환의 단계 수에 있다. 잠재 표현은 encoder를 한 번 거친 결과이고, 재구성은 거기에 decoder를 한 번 더 거친 결과다. 변환이 한 단계 더 일어나면 그만큼 잃는 정보가 생기므로, 입력과 재구성 사이의 상호정보량은 입력과 잠재 표현 사이의 상호정보량보다 작아진다.

이 관계는 병목의 크기를 표현 품질과 곧바로 잇는다. 잠재 벡터의 크기를 줄여 압축을 강하게 하면 입력과 latent code 사이의 상호정보량이 먼저 작아지고, 그보다 클 수 없는 재구성 쪽 정보량이 따라 작아진다. 인코더의 병목 층을 줄여 갈수록 복원 품질이 단계적으로 나빠지는 것이 그 결과다.

부등호의 반대편에도 상한이 있다. 잠재 표현이 담을 수 있는 정보량은 잠재 차원이 담을 수 있는 최대치를 넘지 못하고, 인코더의 효율이 100퍼센트가 아니므로 실제로는 반드시 그보다 작다. 여기서 말하는 최대치는 실제로 계산한 엔트로피 값이 아니라 개념적인 상한이다. 
...
Sparse AE의 L1 정규화는 병목을 256차원처럼 넉넉히 두었을 때 노이즈까지 학습되어 구조가 보이지 않는 상황을 겨냥한다. 일부 차원만 쓰이거나 작은 activation만 일어나도록 유도하는 것이다. Denoising AE는 학습 때 노이즈를 겪게 하여 추론 단계에서 더 강건한 모델을 만든다.


Contractive AE가 더하는 Jacobian 정규화 항은 입력의 변화에 대한 잠재 코드의 변화를 페널티로 만든다. 학습은 그 변화량을 줄이는 쪽으로 진행되고, 결과적으로 입력의 미세한 변동에 덜 민감한 표현을 얻게 된다.

Conditional AE에서 조건 정보를 함께 넣는다는 것은, 예를 들어 MNIST를 복원하면서 클래스 정보도 입력과 출력에 같이 두는 것을 말한다. 이렇게 하면 하나의 모델이 이미지를 생성하면서 클래스도 분류하는 형태가 된다.
...
Super-resolution 모델이 AE 구조를 그대로 쓰는 것은 아니고 변형된 구조를 사용한다. 다만 저해상도 입력을 압축했다가 다시 펼쳐 복원하는 기본 골격은 Autoencoder의 것이며, 그림의 encoder와 decoder 흐름이 그 골격에 해당한다.
![도로 주행 영상이 encoder와 decoder를 거쳐 클래스별 색으로 칠해진 분할 지도로 변환되는 semantic segmentation 구조](images/s111_04.png)


이 그림에서 나뉘는 것은 자동차와 도로, 건물 같은 대상이다. 영상을 세그먼트로 분리해 표현하는 이런 모델도 기본 구조는 Autoencoder이며, 여기서도 encoder가 압축한 표현을 decoder가 펼쳐 픽셀 단위 의미 지도를 만든다.
---
...
두 그림 모두 위가 노이즈를 섞은 입력이고 아래가 Conv AE가 복원한 출력이다. 학습을 마친 AE에 노이즈가 있는 이미지를 통과시켜 노이즈를 걷어 내는 것을 Denoising이라 한다. 노이즈를 일부러 넣어 강건하게 학습시키는 Denoising Autoencoder와는 구분되는 용법이다.
### Keras Conv layer
...
복원된 시계열이 원본 위에 거의 그대로 겹쳐 놓인다. 인코더가 만든 잠재 벡터에 그 구간의 추세와 주기성, 변동성이 담겨 있고, 디코더는 그 벡터만 받아 원래 시계열을 다시 만들어 낸 결과이기 때문이다.
...
각 산점도 제목의 Silhouette Score는 그 잠재 차원에서 군집이 얼마나 뚜렷한지를 말해 준다. 0.7을 넘으면 군집화가 아주 잘 된 것이고, 0.5에서 0.7 사이면 쓸 만하며, 0.5 아래로 내려가면 군집이 꽤 약하고, 0.2에 못 미치면 군집 구조 자체가 보이지 않는다고 읽는다.
