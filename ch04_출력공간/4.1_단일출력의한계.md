...
두 방식은 평균과 분산을 학습한다는 점에서 거의 같아 보이지만 완전히 다른 이야기다. 지금 다루고 있는 것은 출력 공간이며, Gaussian NLL을 손실로 쓴다는 것은 입력을 출력 공간의 분포로 인코딩하겠다는 선언에 해당한다.
...
이 세 가정은 모델이 아니라 데이터가 만족해야 하는 조건이다. 그런데 데이터를 직접 뜯어보아 세 가정이 유지되는지 판별하는 일은 쉽지 않다. 그래서 모델을 학습시킨 뒤 그 결과를 보면서 데이터셋이 이 조건들을 만족하는지 거꾸로 확인하는 방법을 쓴다.


### 가정 확인은 판정이 아니라 정도의 문제다

회귀 모델을 쓰겠다는 결정은 세 가정이 모두 만족한다는 전제 위에서 모델을 돌리겠다는 뜻이다. 전제가 하나라도 어긋나면 학습이 끝난 뒤의 결과 전체가 망가진다. 그런데 데이터를 아무리 들여다보아도 이 조건들이 유지되는지를 직접 가려내기는 어렵다.

그래서 순서를 뒤집는다. 모델을 먼저 학습시키고, 그 학습 과정에 metric 함수를 함께 붙여 값의 움직임을 본다. 유일한 정답 가정이 어느 정도 성립하는지, 등분산성 가정이 어느 정도 성립하는지, 단일 모드 가정이 얼마나 성립하는지를 이 값들로 가늠한다. 결론은 성립과 불성립의 이분법이 아니라 상대적인 정도로 나온다.

단일 모드 가정처럼 직접 확인이 가능한 것도 있다. 국소 분포를 성분이 하나인 분포와 두 개인 분포에 각각 피팅해 보고 어느 쪽이 더 잘 맞는지를 그대로 비교하면, 데이터셋의 모드가 하나인지 그 이상인지를 바로 읽을 수 있다.
...
표에는 가정마다 여러 metric이 올라 있지만, 실무에서 가장 먼저 집어 드는 것은 각 가정마다 하나씩이다. 유일한 정답 가정에는 Local Target Variance가 가장 널리 쓰이고, 등분산성에는 Bin-wise Residual Variance Ratio를, 단일 모드에는 Local GMM BIC gap을 쓴다. 이 세 값만 학습 로그에 올려도 어느 가정이 흔들리고 있는지 대략의 방향은 잡힌다.
...
기본형 Local Target Variance는 모델이 없어도 계산할 수 있다. 입력과 target의 관계만 보기 때문이다. 그래서 학습을 시작하기 전에 데이터셋 자체를 진단하는 용도로 쓸 수 있고, 학습 중에 얻는 학습형 값이 큰지 작은지를 판단할 때 비교의 기준선이 되기도 한다.

이 값이 크게 나오면 회귀 모델을 어떻게 손보아도 더 나은 결과를 얻기 어렵다. 원인이 모델이 아니라 데이터의 구조에 있으므로, 모델 자체를 튜닝하는 일이 의미를 잃는 지점이다.
...
이 계열의 metric은 학습된 표현 공간과 target을 비교하는 형태, 그리고 같은 공간과 residual을 비교하는 형태로 갈라진다. 앞의 것은 모델과 무관하게 데이터셋 자체를 진단하는 쪽에 가깝다. 데이터셋의 성격에 따라 두 값이 서로 다른 신호를 주기도 하므로, 해석에 더 유리한 쪽을 골라 쓰면 된다.
...
첫 두 행을 함께 읽으면 책임 소재가 갈린다. Learned local target variance가 낮다는 것은 입력과 target이 서로 잘 대응하고 있다는 뜻이므로, 그 상태에서 RMSE가 높으면 문제는 데이터가 아니라 모델 쪽에 있다. 둘 다 높으면 원인은 반대로 데이터 구조에 있다.


비교의 기준으로 삼는 베이스라인에는 두 종류가 있다. 하나는 같은 데이터에 걸어 본 다른 모델이고, 다른 하나는 모델을 거치지 않고 데이터셋 자체에서 바로 구할 수 있는 metric 값이다. 어느 쪽과 견주는지에 따라 같은 숫자도 다르게 읽히므로, 기준을 먼저 정해 놓고 표를 읽어야 한다.
...
다중 모드와 이분산을 혼동하지 말라는 주의는 실제로 지키기가 까다롭다. 봉우리가 여럿이어도 서로 겹쳐 버리면 하나의 큰 퍼짐과 구별되지 않기 때문이다. 두 현상을 가르는 일은 상당히 어려우므로 지표 하나에 기대어 결론을 내리지 않아야 한다.
...
손실과 기본 지표는 컴파일 단계에서 지정하고, 진단 metric은 콜백 안에서 계산되어 학습 로그에 얹힌다. 두 경로가 나뉘어 있으므로 진단 지표를 늘리거나 빼도 손실과 기본 지표의 계산은 그대로 유지된다.

콜백을 만들 때 넘기는 값 가운데 이웃의 개수, 구간의 개수, GMM 표본의 크기는 단순한 설정이 아니라 결과를 좌우하는 하이퍼파라미터다. 이웃을 잘못 잡으면 그 위에서 계산되는 값들이 함께 망가진다.
...
여섯 개의 metric은 같은 데이터를 서로 다른 각도에서 재기 때문에 값들이 때로 모순처럼 보인다. 그럴 때는 지표 하나로 결론을 내지 말고 더 많은 지표가 지지하는 쪽으로 해석해야 한다. 어디까지나 상대적인 비교라는 점을 잊지 않아야 한다.
...
반대 방향의 오해도 짚어 둘 만하다. 해의 개수는 팔의 자유도에 따라 달라진다. 앞의 로봇 팔에서 해가 둘인 것은 관절이 두 개일 때의 이야기이고, 팔이 옆으로 도는 축을 하나 더 가지면 같은 위치를 만드는 자세는 무한히 많아진다. 역문제라는 말은 해의 개수가 아니라 접근하는 방향을 가리킨다.
...
회귀는 이 산점도의 한가운데를 지나는 평균선을 찾는다. 그런데 두 그림 모두 입력이 커질수록 그 평균선 주위의 퍼짐이 함께 커진다. 값 하나만 내놓는 예측은 이 퍼짐의 변화를 담을 수단이 없으므로, 이런 데이터에서는 어떤 값을 내놓아도 실패한다.
...
이 분포의 평균은 $0.6 \times (-2) + 0.4 \times (+2) = -0.4$ 이다. 그런데 $y \approx -0.4$ 부근에는 **데이터가 거의 존재하지 않는다.**

![두 Gaussian의 혼합 $0.6\,\mathcal{N}(-2,0.5^2) + 0.4\,\mathcal{N}(2,0.5^2)$ 의 밀도 곡선. 평균 −0.4 위치에는 밀도가 거의 없다](images/s213_02.png)
<!-- 2026-09-04 정정: 평균을 0.11 → −0.4 로 고치고 그림 s213_02.png 도 다시 그렸다.
     0.11 은 산술과 맞지 않았다. 슬라이드 213 은 아직 0.11 이므로 강의자료도 함께 고쳐야 한다. -->


*그림 4-8. 평균이 데이터가 없는 자리를 가리키는 경우.*

평균을 표시한 세로선은 두 봉우리 사이의 골짜기에 그어진다. 이 값은 왼쪽 분포에도 오른쪽 분포에도 속하지 않는다. 다중 모드 분포에서 평균을 구하는 일이 회귀 모델의 대표적인 실패 사례로 꼽히는 이유가 이 그림에 그대로 드러난다.
...
Interval Width는 모델이 예측한 분산의 크기를 그대로 반영한다. 구간이 좁으면 분산을 작게 본 것이고, 넓으면 크게 본 것이다. Coverage Gap은 Coverage와 목표값의 차이만 따로 떼어 하나의 숫자로 지표화한 것이다.


Coverage는 목표치와 어긋난 방향까지 함께 읽는다. 목표보다 작으면 구간을 좁게 잡아 과소신뢰한 것이고, 목표보다 크면 구간을 넓게 잡아 과대신뢰한 것이다.

이 지표들이 있어서 모델이 내놓은 분산을 되짚어 볼 수 있다. 이분산 회귀는 분산을 예측하지만 그 분산이 실제로 맞는지는 스스로 말해 주지 않는다. 보정 지표는 예측된 분산을 다시 한 번 검산하는 절차에 해당한다.
...
예측 구간의 경계를 보면 군데군데 찌그러진 곳이 있다. 데이터가 충분하지 않아 완전히 학습되지 못한 흔적이다. 그럼에도 평균과 분산 두 값만으로 원점 부근은 좁고 양끝은 넓은 데이터의 퍼짐 구조를 되살려 냈다는 점은 확인할 수 있다.
...
점들은 대각선을 중심으로 아래위로 오르내릴 뿐 크게 벗어나지 않는다. 어떤 신뢰 수준을 지정하든 실제 데이터가 대체로 그만큼 구간 안에 들어왔다는 뜻이고, 95% 구간에서도 마찬가지다. 이 모델은 coverage가 잘 맞는 편이다.
...
Sharpness 히스토그램은 예측된 $\sigma$ 자체의 분포다. 값이 작은 쪽에 몰려 있으면 예측이 전반적으로 날카롭다는 뜻이고, 큰 값까지 넓게 퍼져 있으면 좁게 예측한 구간과 넓게 예측한 구간이 데이터 안에 함께 존재한다는 뜻이다.
...
강의는 현실의 데이터에 등분산인 경우가 거의 없으므로 이분산성이 있느냐가 아니라 어느 정도냐를 물어야 한다고 지적했다. 이분산성이 남아 있는 한 출력을 하나의 값으로 추정하는 방식은 구조적으로 실패하며, 그래서 퍼짐을 함께 내놓는 출력 형태가 필요해진다.
...
커버리지는 예측 구간 안에 실제 데이터가 얼마나 들어오는지를 재는 지표다. 그런데 분위수 회귀는 애초에 그 커버리지를 맞추도록 학습되므로 경험적 커버리지가 대각선 위에 놓이는 것은 예정된 결과에 가깝다. 이 그림이 좋다는 사실만으로 구조를 잘 설명했다고 볼 수는 없다.
...
강의는 보정 지표들이 가우시안 분포를 전제로 만들어졌다는 점을 강조했다. 평균에서 ±1.96 시그마를 95% 구간으로 보는 방식이 그 예다. 꼬리가 두껍거나 이상치가 섞인 분포에서는 커버리지가 크게 어긋나고 지표의 의미도 함께 약해진다.
...
표준화 잔차는 정규분포에 가까울수록 모델이 잘 동작한다고 본다. PIT는 값들이 균등분포에 가깝게 퍼졌는지를 보고, Sharpness는 구간 폭의 분포다. 폭이 작은 쪽에 몰려 있으면 날카로운 예측이고, 큰 값까지 벌어져 있으면 넓은 구간이 함께 섞여 있다는 뜻이다.
