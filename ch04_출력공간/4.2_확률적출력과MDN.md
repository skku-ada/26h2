...
출력 쪽에 로지스틱 함수나 그 변형인 softmax를 붙이면 분류 모델이 되지만, 이것도 하나의 값을 내보내는 구조 위에 확률 변환을 얹은 것이다. 분류 모델 역시 회귀 모델의 변형일 뿐이므로 단일 정답 가정에서 벗어나지 않는다. 층을 아무리 넓게 쌓고 깊게 쌓아도 출력이 결정론적이라는 성질은 그대로 남는다.
...
표에 적은 정답의 개수는 고정된 값이 아니다. 로봇 팔의 해가 둘로 갈라진다는 설명은 관절이 두 개일 때의 이야기이고, 관절 수가 늘어나면 같은 목표 위치를 만들어 내는 각도 조합도 함께 늘어난다. 정답의 개수는 문제의 구조가 정하는 것이지 미리 정해진 상한이 있는 것이 아니다.
...
봉우리가 두 개인 분포라면 신경망은 평균 둘, 분산 둘, 그리고 두 봉우리의 높이에 해당하는 혼합계수 둘을 내놓는다. 위치도 폭도 높이도 서로 다른 두 분포를 결합해 전체 합이 1이 되는 하나의 확률분포를 만드는 것이 그림 오른쪽의 결과다.
...
무작위 샘플링의 결과는 뽑을 때마다 달라진다. 그래서 한 번 뽑은 값 하나로는 그 입력에서의 전체 분포를 대표할 수 없다. 같은 입력에 대해 여러 번 뽑아 보아야 값들이 어느 위치에 얼마나 모여 있는지가 드러난다.

그럼에도 하나의 값을 반드시 결정해야 하는 상황이 있다. 이때 쓰는 것이 혼합계수가 가장 큰 성분의 평균을 그대로 답으로 삼는 방식이다. 가장 가능성이 높은 모드를 골라 그 중심을 답으로 내보내는 셈이다.

상위 성분을 여러 개 내보내는 방식은 성격이 조금 다르다. 결정된 값이 필요하면서 동시에 답의 후보를 함께 보고 싶을 때 쓴다. 혼합계수가 큰 순서대로 성분을 골라 그 평균들을 나란히 제시하면, 역문제처럼 해가 여러 갈래인 문제에서 가능한 답들을 함께 놓고 판단할 수 있다.
...
이름이 비슷해 헷갈리기 쉬우므로 한 번 정리해 둔다. 앞 소절에서 다룬 GMM은 Gaussian Mixture Model이고, 이 소절의 MoG는 Mixture of Gaussian이다. 두 이름은 순서만 뒤집힌 듯 보이지만 가리키는 모델이 다르다.

허들 모델이 필요한 자리는 값보다 사건이 먼저 존재하는 데이터다. 거래가 있어야 거래액을 추정할 수 있고, 보험에 가입해야 청구 금액이 존재하며, 클릭이 있어야 그 뒤의 구매 금액을 말할 수 있다. 발생하지 않았을 때와 발생했을 때의 조건 자체가 다르므로 두 경우를 한 분포 안에서 나누어 다룬다.
...
그림의 신경망 자체는 보통의 순전파 구조와 다르지 않다. 달라지는 것은 출력을 읽는 방식이다. 성분이 세 개라면 평균 셋, 분산 셋, 혼합계수 셋을 합쳐 아홉 개의 값이 모수 벡터 θ를 이루고, 그 θ가 오른쪽의 다봉 조건부 분포 하나를 정의한다.
...
해가 여러 개인 문제라고 해서 곧 역문제인 것은 아니다. 역문제의 정의는 출력값으로부터 입력값을 거꾸로 추정하는 것이고, 그 과정에서 해가 여러 갈래로 갈라지는 경우가 대표적인 사례일 뿐이다. MDN이 겨냥하는 것이 바로 그 다중해 상황이다.
...
세 개의 봉우리는 저가·중가·고가라는 세 가격대에 대응한다. 같은 가격대 안에서도 적절한 지점을 벗어나면 판매량이 줄어들기 때문에 각 봉우리가 좌우로 완만하게 흘러내린다. 어느 가격에 내놓으면 어느 정도 팔릴지를 읽는 것이 이 분포의 쓰임이다.
...
축을 뒤집으면 정답의 개수가 구간마다 달라진다. 양 끝에서는 정답이 하나뿐이지만 가운데로 갈수록 둘, 셋으로 늘고 많게는 다섯 갈래까지 생긴다. 회귀 곡선은 그 갈래들의 평균만 따라가므로, 정답이 여럿인 구간에서 데이터가 없는 빈자리를 지난다.
...
성분 수를 5로 둔 것은 임의의 선택이 아니다. 이 모델이 학습할 역문제 데이터는 입력 구간에 따라 정답이 최대 다섯 갈래까지 갈라지므로, 그 모드를 모두 담으려면 성분이 최소 다섯 개는 있어야 한다. 세 출력의 shape이 모두 5인 이유가 여기에 있다.


여기에 해석상의 이유도 겹친다. 다섯 성분이 각자 자리를 잡으면 어느 성분이 어느 가지를 맡았는지가 뚜렷하게 보이므로 결과를 설명하기 좋아진다. 성분을 더 늘리면 학습이 흔들리는 일은 줄지만 성분별 역할이 흐려져 설명하기가 어려워진다.
...
이 손실이 재는 것은 분포의 모양 자체가 아니라 정답이 그 분포 안에 얼마나 들어와 있는가이다. 모델은 입력마다 여러 개의 분포를 만들어 내고, 그 분포들이 타겟 값을 잘 덮을수록 가능도가 커진다. 학습은 이 가능도에 음의 로그를 씌운 값이 작아지는 방향으로 진행된다.
...
다만 이 문제가 늘 나타나는 것은 아니다. 그냥 계산해도 별 탈이 없는 경우가 많고, 분포의 범위가 아주 넓어 target 이 성분에서 멀리 떨어지는 특수한 상황에서만 불안정해진다. 자주 겪는 일은 아니지만 미리 막아 두는 편이 안전하다.
...
여기서 동치는 값이 같다는 뜻이 아니다. 세 표현이 찾아내는 파라미터가 같다는 뜻이므로 등호로 이어 쓰면 안 되고 동치 기호로 써야 한다.

그래서 같은 학습을 두고 최대가능도 추정을 한다고 말하기도 하고, 로그가능도를 최대화한다고 말하기도 하며, KLD를 최소화한다고 말하기도 한다. 표현이 갈릴 뿐 하는 일은 하나다.
...
KLD는 두 분포 사이의 관계를 재는 양이며, 통신에 빗대면 보낸 정보량과 받은 정보량의 차이, 즉 손실된 정보량의 기댓값에 해당한다. 이 관점에서 보면 앞에서 갈라 놓은 두 항의 성격이 분명해진다.

첫째 항은 데이터에 대한 엔트로피다. 입력과 출력의 관계를 있는 그대로 표현한 값이며 모델의 파라미터가 들어가지 않는다. 통신에 비유하면 보내는 쪽의 정보량에 해당하고, 데이터셋이 정해지는 순간 상수로 고정되므로 최적화에서는 의미를 갖지 못한다.

둘째 항이 모델이 찾아낸 분포에 대한 항이며, 결국 모델의 평균 로그가능도가 된다. 따라서 KLD를 최소화한다는 것은 상수인 앞 항을 버리고 뒤 항만 최소화하는 파라미터를 찾는 일이 된다.

앞서 Cross Entropy를 손실로 쓸 때 가져다 쓴 것도 바로 이 뒤 항이었다. 이름이 달랐을 뿐 최소화하는 대상은 같다.
...
오른쪽 단면에서 봉우리의 높이가 서로 다른 것은 혼합계수의 차이다. 그 위치에서 데이터를 실제로 지나가는 성분은 혼합계수가 크게 학습되고, 데이터가 없는 곳을 지나는 성분은 작은 값을 갖는다. 입력이 옮겨가면 큰 값을 갖는 성분도 함께 바뀐다.
...
이 함수가 돌려주는 것은 손실 그 자체가 아니라 손실의 재료다. 각 성분이 만들어 낸 분포에 정답을 넣어, 그 성분이 정답을 얼마나 지지하는지를 성분별로 뽑아 놓은 값이기 때문이다. 이 기여도들을 혼합계수와 합치고 하나로 줄여야 비로소 손실이 된다.
...
하나의 입력에서 값을 한 번만 뽑으면 분포의 모양이 드러나지 않는다. 같은 $x$ 마다 열 번씩 샘플링했기 때문에 푸른 점이 각 가지 위에 여러 겹으로 쌓이고, 붉은 점으로 그린 정답 데이터의 퍼짐까지 그대로 재현된다.
...
혼합계수 패널을 보면 어느 구간에서 어떤 성분이 쓰이는지 읽을 수 있다. 계수가 0 가까이 내려간 구간에서는 그 성분에서 샘플이 뽑히지 않으므로, 같은 구간의 분산이 크게 튀어도 결과에는 영향이 없다. 여러 가지가 겹치는 구간에서는 다섯 성분의 계수가 비슷해진다.


학습 중간을 들여다보려면 관찰 지점을 미리 정해 두어야 한다. 실습에서는 배치 반복 안에서 검증 데이터를 함께 넣어 확인하고, 일정 스텝마다 한 번씩 네 패널을 출력한다. 초반에는 평균 곡선들이 서로 겹쳐 있다가 스텝이 쌓이면서 하나씩 벌어지고, 3천 스텝 부근에서 다섯 곡선이 각자 하나의 가지를 따라가기 시작한다.  
...
성분을 늘리면 학습이 망가지는 일은 줄지만, 늘린 성분이 모두 쓰이는 것은 아니다. 일부만 가지를 맡고 나머지는 혼합계수가 0에 가까운 채 데이터 사이를 지나간다. 성분별 역할이 흐려져 해석이 어려워지고, 노이즈까지 학습해 오히려 성능이 떨어질 수도 있다.
...
NLL은 작을수록 좋으므로 음수 쪽으로 더 내려간 곡선이 더 나은 모델이다. $K=1$ 은 위쪽에서 멈추고 $K=2$ 의 곡선이 가장 낮은 값까지 내려간다. $K$ 를 3, 5로 더 늘린 쪽은 같은 데이터에서 약간의 과적합이 생겨 오히려 손실이 높아졌다.
...
컴포넌트를 늘린다고 성능이 계속 좋아지지는 않는다. 어느 지점을 넘어서면 오히려 과적합이 생기면서 Validation Loss 가 다시 올라간다. 따라서 K 는 크게 잡는 방향이 아니라 Validation NLL 이 가장 낮아지는 지점을 찾아 고르는 것이 일반적이다.
...
과제 수행 범위는 다음과 같다. 실제 데이터셋에 MDN 을 적용해 K 가 결과에 미치는 영향을 확인하는 심화 과제 4-5 는 기본 과제이므로 모두 수행하고, 이어지는 심화 과제 4-6 과 4-7 은 둘 중 하나를 선택해 제출한다.
