...
학습을 마쳤다는 것은 모델이 데이터의 구조를 찾아냈다는 뜻이고, 그 처리의 흔적이 파라미터에 남는다는 뜻이다. 그래서 이 절이 추적하는 대상은 두 가지다. 하나는 파라미터 자체의 분포이고, 다른 하나는 노드의 출력, 곧 활성화된 값의 분포다.
...
이 추적으로 확인하는 것은 차원의 수만이 아니다. 모델의 크기가 데이터에 견주어 적절한지, 각 layer의 노드 수가 적절한지, 그리고 어느 지점에서 정보가 급격히 망가지는 layer가 있는지를 층을 따라가며 함께 본다.

절차는 절 전체에서 같은 형태로 반복된다. layer의 weight를 특이값 분해하고 그 분포를 엔트로피로 요약해 effective rank를 얻으면, 그 layer에서 모델이 실제로 쓰고 있는 내재 차원이 드러난다.
...
- $\alpha < 2$ : 소수의 큰 특이값이 나머지를 압도해, 에너지가 몇 방향에 몰린다. 과적합, 붕괴여부 검토가 필요하다.
- $\alpha \approx 2 \sim 4$ : 균형 잡힌 상태다. 학습과 일반화가 모두 좋다.
- $\alpha > 6$ : 고르게 퍼짐, 비슷한 크기를 나눠 갖는다. 학습부족 검토가 필요하다.

$\alpha$ 가 클수록 특이값이 고르게 퍼지고, 작을수록 소수에 몰린다. 이 방향은 위의 Hill Estimator 식에서 곧바로 따라온다 — 값들이 서로 비슷하면 로그 비의 합이 작아져 $\alpha$ 가 커지고, 몇 개가 압도하면 그 합이 커져 $\alpha$ 가 작아진다.

이 진단의 실용적 가치는 **검증 데이터 없이 학습 품질을 진단**할 수 있다는 데 있다. 별도의 검증 집합을 소모하지 않고 weight만 들여다보아 상태를 짐작할 수 있다.


다만 $\alpha$ 값을 읽을 때는 추정 절차를 함께 확인해야 한다. 꼬리 구간을 어디까지로 잡는지와 어떤 추정량을 쓰는지에 따라, 같은 layer의 $\alpha$ 가 위의 기준과 반대 방향으로 읽히는 경우가 실제로 나타난다. 값 하나만 떼어 판정하기 전에 특이값 분포를 직접 그려 확인하는 편이 안전하다. 
...
이 비율을 보는 목적은 layer가 주어진 자원을 어느 정도 쓰고 있는지를 확인하는 데 있다. 모델을 구성한 뒤 layer별로 훑었을 때 상대적으로 값이 작은 layer가 있다면, 층 사이의 균형이 맞지 않거나 그 layer에 맡겨진 일이 별로 없다는 뜻이다.

이런 layer에는 놀고 있는 노드가 생긴다. 의미 없는 파라미터가 거의 0에 가까운 값으로 남고, 그 결과 특이값 중에도 0에 가까운 것이 만들어진다. 뒤에서 다룰 condition number의 급등이 여기에서 시작한다.
...
평탄한 경우 erank는 $d = \min(\text{w.shape})$ 근처까지 올라간다. 여기서 $d$ 는 입력 쪽과 출력 쪽 노드 수 가운데 작은 값이며, weight가 가질 수 있는 최대 차원이므로 특이값도 그 개수까지만 나온다.

반대편 극단에서는 특이값 하나만 남고 나머지가 사라져 erank가 1이 되며, 이는 그 layer의 노드가 사실상 죽어 버린 상태다. 스펙트럼을 읽을 때는 두 극단 사이의 어디에 있는지를 layer의 명목 차원과 견주어 판단한다. ⚠확인
...
### $\kappa$ 급등의 원인을 끝까지 좁히기

앞의 히스토그램만으로는 `enc2` 의 $\kappa$ 가 왜 그렇게 커졌는지가 확정되지 않는다. 모든 layer의 특이값을 한 그림에 겹치고 세로축을 로그 스케일로 바꾸면 원인이 드러난다. 다른 layer의 특이값이 일정 크기 아래로 내려가지 않는 데 비해, `enc2` 에서만 특이값이 $10^{-4}$ 수준까지 떨어진다. $\kappa$ 는 가장 큰 특이값을 가장 작은 특이값으로 나눈 값이므로, 극단적으로 작은 이 분모 하나가 값을 밀어 올린 것이다.

원인은 여기서 한 단계 더 거슬러 올라간다. 인코더는 layer가 둘인데 디코더는 하나인 비대칭 구조에서, 같은 128 노드의 layer라도 한쪽은 111개를 쓰고 다른 쪽은 99개만 쓴다. 일이 상대적으로 적은 layer에서는 일부 뉴런이 제대로 작동하지 않고, 그 뉴런에 걸린 파라미터가 거의 0으로 남아 아주 작은 특이값이 만들어진다. 곧 $\kappa$ 의 급등은 그 자체가 원인이 아니라 구조의 불균형이 남긴 흔적이다.

### 세 가지 대응이 각각 무엇을 고치는가

대칭형 구조로 바꾸거나 Normalization과 Skip Connection을 넣는 것은, 특정 layer에만 일이 몰리지 않도록 구조 쪽을 손보는 방법이다.

Weight decay와 L2 정규화는 파라미터가 서로 비슷한 크기에서 만들어지도록 눌러 준다. 유독 작은 값 하나가 생기는 상황 자체가 줄어들므로 이런 형태가 덜 나타난다.

Low-Rank 행렬 분해는 학습 도중 erank를 보면서 필요 없는 차원을 잘라 내는 방법이다. 아주 작은 특이값이 애초에 남지 않으므로 모델이 안정된다. 
...
그 신호는 두 갈래로 읽힌다. 모델이 데이터의 정보량에 비해 그 layer를 너무 크게 잡은 경우이거나, 특정 layer의 학습이 잘못되어 망가진 경우다. 어느 쪽인지는 layer의 크기와 학습 조건을 함께 놓고 판단한다.
...
원인 쪽에도 갈래가 있다. learning rate가 상대적으로 큰 경우가 대부분이지만, 입력 데이터의 변동성이 지나치게 크거나 데이터 자체가 음수 쪽으로 치우친 경우에도 뉴런의 출력이 계속 음수에 머무른다.

학습 초기에 뉴런이 죽는 것을 막으려고, learning rate를 처음부터 크게 주지 않고 작은 값에서 천천히 올렸다가 다시 내리는 warmup 계열의 스케줄러를 쓰기도 한다. 모델이 커질수록 일부 뉴런이 초기에 죽는 일이 잦아지므로 더 자주 쓰인다.
...
다만 우측으로 긴 꼬리는 한 가지로만 읽히지 않는다. 소수의 강한 패턴이 지배하는 경우일 수도 있고, 과적합이 진행되어 출력값 자체가 커진 경우일 수도 있다. 활성값이 어느 범위에 머무는지를 함께 보아야 두 경우가 갈린다.
...
재해석의 요지를 학습률 쪽에서 보면 이유가 분명해진다. loss 평면이 거칠면 학습률이 클 때 어느 방향으로 뛸지 알 수 없고, 부분적으로는 역방향으로도 움직여 학습이 지연된다. 평면이 매끄러워지면 방향을 안정적으로 잡고 나아갈 수 있으므로 더 큰 학습률을 감당하게 된다.
...
대시보드에서 확인하는 것은 절대적인 수치가 아니라 차이다. 테스트 정확도가 떨어졌을 때 학습 시점의 분포와 무엇이 달라졌는지를 대조하면, 성능 숫자만으로는 보이지 않던 layer 단위의 변화가 드러난다.

히스토그램을 읽는 기준은 단순하다. 어떤 layer의 활성값이 대부분 0에 몰려 제대로 된 값이 거의 남아 있지 않다면 그 layer는 사실상 동작하지 않는 것이고, 모든 값이 0으로만 나온다면 아예 멈춘 것이다.

이 대시보드는 다음 소절로 이어진다. layer별 activation에서 뽑은 rank와 엔트로피를 깊이 순으로 나란히 놓으면, 어느 layer에서 값이 갑자기 줄어드는지를 층 사이의 비교로 잡아낼 수 있다.
...
이 곡선을 읽는 목적은 압축의 속도를 보는 데 있다. 깊이를 따라가며 정보가 적절한 수준으로 압축되는지 확인하고, 어느 지점에서 갑자기 크게 압축되면 그만큼 사라지는 정보가 많아진다고 본다. 정보량을 유지하면서 구조를 설계하는 기준이 여기에서 나온다.
...
엔트로피의 높낮이 자체는 좋고 나쁨의 판정이 아니라 상태의 기술이다. 값이 낮을 때 특화로 읽을지 Dead Neuron으로 읽을지는 그 layer의 크기에 따라 갈리고, 값이 높을 때도 정보량이 많아 모든 노드가 일하고 있는 것인지를 따로 확인해야 한다.
...
### Activation을 모으는 sub model

layer의 활성값은 모델 바깥에서 바로 꺼낼 수 없다. 원래 모델의 입력을 그대로 입력으로 삼고, 보고 싶은 layer들의 출력을 리스트로 묶어 출력으로 삼는 sub model을 따로 만든다. 여기에 테스트 데이터나 학습 데이터를 넣으면 지정한 layer들의 활성값이 한 번에 나온다.

이 방식은 Dead Neuron 분석과 layer별 erank 분석에 똑같이 쓰인다. weight를 대상으로 한 분석은 학습이 끝난 모델에서 파라미터를 꺼내 오면 그만이지만, 활성값을 대상으로 한 분석은 모델에 데이터를 흘려 보아야 값이 생긴다. 같은 erank라도 무엇을 넣어 계산했는지에 따라 접근이 갈리는 이유가 여기에 있다.
...
### erank를 기준으로 구조를 조정할 때

뉴런 수에 견주어 erank가 뚜렷하게 작다면 그 모델은 크다고 볼 수 있다. 이때 각 layer의 폭을 erank에 일정 배수를 곱한 값으로 다시 잡으면, 목표 성능과 모델 크기를 함께 조정하는 출발점이 된다.

엔트로피가 낮아진다는 것은 값들이 넓게 벌어지지 않고 비슷한 크기로 채워진다는 뜻이며, 중복이 걷혔다는 신호로도 읽는다. 판단의 기준은 줄인 뒤의 비교에 있다. erank가 눈에 띄게 작아졌는데도 성능이 비슷하게 유지된다면, 원래 모델의 파라미터가 상대적으로 컸다는 뜻이다. 그렇다면 남은 불필요한 차원을 더 줄일 여지가 있고, 다음 단계로 연결 자체를 줄이는 pruning과 Lottery Ticket을 검토하게 된다.
...
이 관점에서 보면 모델을 이루는 수많은 노드와 연결 가운데 핵심이 되는 연결이 따로 존재한다는 말이 된다. 모델이 특별한 초기값과 특별한 경로를 함께 가질 때 좋은 결과에 이른다는 것이 이 가설의 요지다.
...
기본 pruning과 갈리는 지점은 3단계다. 일반적인 pruning은 연결을 제거한 뒤 그 상태에서 이어서 학습하지만, IMP는 남은 weight를 초기값으로 되돌린 다음 다시 학습한다. 되돌리는 이 한 단계가 두 방법의 차이 전부다.
...
모델 효율을 높이는 접근은 크게 두 축으로 나뉜다. 하나는 지금까지 본 것처럼 모델 자체를 줄이는 방향이고, 다른 하나는 Knowledge Distillation처럼 teacher 모델의 결과를 student 모델이 학습하게 하여 작은 모델의 성능을 끌어올리는 방향이다.
...
첫 번째 항목은 뒤집어 읽을 수도 있다. 같은 데이터로 학습한 두 모델의 분포가 크게 달라진다면 그 모델은 구조적으로 민감하다는 뜻이며, 모델과 데이터 사이에 맞지 않는 부분이 있다는 신호로 볼 수 있다.
