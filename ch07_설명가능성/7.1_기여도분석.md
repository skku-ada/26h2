...
<!-- 원문 불일치 확인 필요: 원문 슬라이드 462 는 σ ∈ {0, 0.05, 0.1, 0.2, 0.3} · T ∈ {1, 5, 10, 30, 100} 으로 적었으나, 노트북 코드와 슬라이드 그림은 σ=0.5 · T=300 까지 포함한다 -->
...
<!-- 원문 불일치 확인 필요: 노트북 출력 화면(s484_03)에는 임계 0.3 옆에 'Adebayo' 라는 표기가 찍혀 있으나, 원문 슬라이드 481 은 이 임계를 탐지 워크플로우 4단계의 기준으로만 제시하고 누구에게도 귀속시키지 않는다. Adebayo 2018 은 saliency map 의 sanity check 논문이다 -->
