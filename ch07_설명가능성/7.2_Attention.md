...
일반화 이전과 이후의 차이는 입력의 개수에서도 드러난다. 이전의 Attention은 인코더와 디코더의 상태만 가지고 score를 계산했다. 일반화된 형태에서는 입력이 Query와 Key와 Value 셋으로 늘어나며, Key는 어떤 Value를 찾는 것이 좋은지를 알려 주는 자리를 맡는다.
...
### Self-Attention은 구조가 아니라 활용 방식이다

Self-Attention은 새로운 층이 아니다. 앞에서 본 Attention Head와 구조가 똑같고, 그 Head에 무엇을 넣느냐만 다르다. 그래서 Self-Attention은 구조의 이름이라기보다 활용 방식의 이름에 가깝다.

같은 값을 넣는다고 할 때의 같다는 말도 정확히 새길 필요가 있다. Attention 연산에 들어가는 세 벡터가 같은 것이 아니라 그 벡터들이 나온 원천이 같다는 뜻이다. 세 값은 여전히 서로 다른 투영 층을 거쳐 만들어지므로 연산에 들어가는 시점에는 서로 다른 벡터다.

원천이 같다는 것이 무엇을 가리키는지는 다루는 데이터에 따라 달라진다. 문장을 다루면 같은 시퀀스 안의 토큰들이고, Feature 벡터를 다루면 특정 위치에 놓인 각각의 Feature다.
...
용어를 좁혀 두면 이 관점이 더 분명해진다. 참조는 의존의 부분집합이다. Attention이 재는 것은 한 요소의 Query가 다른 요소의 Key에 두는 참조 강도의 분포이며, 분포이므로 확률이다. 곧 특정 요소의 표현을 갱신할 때 다른 요소들을 얼마나 참조하는가를 나타낸 값이다.
...
두 도구의 산출물 형태가 갈리는 까닭은 분해의 단위에 있다. Attribution은 출력값 하나를 입력들에게 나누어 주는 일이므로 결과가 벡터 하나로 끝난다. 반면 Attention은 Query마다 자기 분포를 따로 가지므로, 그 분포들이 행으로 쌓여 행렬이 된다.
...
두 쓰임 가운데 패딩은 배치 단위로 문장을 넣을 때 생긴다. 한 배치 안에서 문장 길이가 서로 다르면 짧은 문장의 뒤쪽에 자리를 채우는 토큰을 붙이는데, 이 토큰이 참조되지 않도록 그 자리의 점수를 가리는 것이 패딩 마스킹이다.
...
경로가 길다는 것이 무엇을 뜻하는지 짚어 둘 필요가 있다. RNN은 시퀀스 길이만큼 정보를 차례로 전달하므로, 뒤쪽 위치는 앞쪽 정보를 그대로 받지 못하고 여러 단계를 거치며 변형된 형태로만 받는다. 경로가 짧다는 것은 모든 요소 쌍이 곧바로 연결된다는 뜻이다.
...
덧붙이면, 언어 모델을 빼고 나면 실무에서 다루는 데이터는 대부분 정형 데이터다. 그래서 Transformer를 정형 데이터에 맞추려는 시도가 이어졌고, 그 과정에서 모델들이 여러 갈래로 분화했다. 뒤의 소절이 그 분화를 다룬다.
...
Attention 층에 값을 넣을 때는 원래 Query와 Key와 Value 셋을 각각 지정한다. 다만 Self-Attention에서는 셋의 원천이 같으므로 같은 입력을 넘겨 주면 Query 쪽과 Key 쪽과 Value 쪽 투영에 모두 그 값이 전달된다.

Attention 층에서 받는 출력은 둘이다. 하나는 Attention을 거친 결과값으로 시퀀스 길이와 표현 차원을 그대로 갖는다. 다른 하나는 Attention score 행렬로, Feature 수만큼의 행과 열을 가진 정사각 행렬을 Head 수만큼 갖는다. 관계를 읽는 데 쓰는 것은 뒤쪽이다.

결과값은 전역 평균 풀링을 거친다. 이때 시퀀스 축이 사라지고 표현 차원만 남으므로, 그 뒤에 밀집 층을 붙여 예측값 하나를 내보낼 수 있다.

모델은 하나로 만들되 둘로 나누어 쓴다. 하나는 입력에서 예측값을 내는 모델이고, 다른 하나는 같은 입력에서 Attention score를 되돌려 주는 모델이다. 관계를 보려면 두 번째 모델이 필요하다.
...
순서가 없다고 해서 자리 정보를 아예 빼도 되는 것은 아니다. 위치 정보를 넣지 않으면 모델이 제대로 학습되지 않는다. 그래서 Feature의 자리를 벡터로 바꾸어 주는 임베딩 층을 따로 둔다.

이 층은 고정된 규칙이 아니라 학습 대상이다. Feature의 자리마다 어떤 코드를 할당하는 것이 좋은지를 학습으로 정하며, 따라서 이 층에도 갱신되는 파라미터가 있다. 위치 인코딩을 계산식으로 붙이는 방식과 갈리는 지점이 여기다.

만들어진 자리 벡터는 입력 쪽 투영 층을 통과한 Feature 벡터와 더해진 뒤 Attention 층으로 들어간다. Attention이 보는 것은 Feature의 값만이 아니라 값과 자리 정보가 합쳐진 벡터다.
...
여기서 병렬이라는 말을 오해하지 않아야 한다. 각 Head가 같은 벡터를 그대로 복제해 받는 것이 아니라, 입력 벡터를 Head 수만큼 쪼개어 나누어 받는다. 그래서 Head를 늘리면 Head 하나가 다루는 차원이 그만큼 줄어든다.
...
이 중복은 큰 모델을 작은 모델로 옮기는 작업에서 다시 확인된다. 언어 모델을 아주 크게 만들어 두고 그것을 작은 모델로 Knowledge Distillation 하면 작은 모델이 상당히 잘 학습된다. 모델이 크다는 것이 그 크기만큼 일을 하고 있다는 뜻은 아니며, 큰 모델 안에 잉여 상태로 남아 있는 Head와 층이 적지 않다는 뜻이다.

그래서 어느 Head가 실제로 일하고 있고 어느 층이 제 몫을 하고 있는지를 따로 재어 볼 필요가 생긴다. 각 Head가 잘 학습되었는지, 각 층이 잘 학습되었는지는 학습이 끝난 뒤의 Attention 행렬을 가져다 측정하는 방식으로 확인한다. 해석에 앞서 그 Head를 해석 대상으로 삼아도 되는지를 먼저 거르는 절차인 셈이다.
...
### Effective Rank가 재는 것

Effective Rank는 벡터의 차원 가운데 실제로 몇 개 정도가 쓰이고 있는지를 수치로 만든 값이다. 차원을 넉넉히 잡아 두더라도 모델이 그 전부를 쓰는 것은 아니므로, 이름만 있는 차원과 실제로 정보를 나르는 차원을 갈라 보자는 발상이다.

측정은 특이값 분해로 한다. 대상 행렬을 분해한 뒤 쓸 만한 크기를 가진 특이값이 몇 개나 남는지를 센다. 남은 특이값이 적다면 그 Head는 좁은 부분공간만 쓰고 있다는 뜻이며, 그것이 곧 그 Head가 실제로 사용되고 있는지 아닌지를 판단하는 근거가 된다.  
...
세 도구 가운데 Attribution과 Attention은 둘 다 의존이라는 같은 용어를 쓴다. 그러나 의존의 양끝이 다르다. Attribution은 입력과 출력 사이의 의존이고, Attention은 요소와 요소 사이의 의존, 곧 참조다.

쓰임도 그만큼 갈린다. Attribution은 baseline과 모델에 종속되므로 학습된 모델에서 입력과 출력이 어떻게 묶여 있는지를 확인하는 데 쓰고, Attention은 각 Feature가 서로를 어떤 방향으로 참조하는지를 확인하는 데 쓴다.
