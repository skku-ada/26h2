...
세 번째 관점을 택한다는 것은 변수를 고르고 전처리하는 모든 작업을 가설을 제안하는 일로 읽겠다는 뜻이다. 분석가는 데이터에 이미 있는 것을 발견하거나 성능에 유리한 것을 고르는 데 그치지 않고, 그 선택으로 검증할 주장을 하나씩 세워 나간다.

그래서 이 관점에서 Feature Engineering은 가설을 교체하는 작업으로 설명된다. 원시 데이터가 그대로여도 전처리를 달리하면 결론이 달라지며, 달라진 결론은 데이터가 아니라 교체된 가설에서 나온 것이다.

Feature를 새로 만드는 일도 같은 구조를 갖는다. Feature를 하나 추가한다는 것은 검증해야 할 주장을 하나 추가하는 것이고, 최종 결론은 그렇게 쌓인 가설들 위에서 만들어진다. 이 절이 검토하는 대상은 그 가설들이다.
...
세 차원은 서로 다른 종류의 주장을 담는다. 변수 선택은 어떤 차원이 결과에 관여하는가에 대한 주장이고, 함수 형태는 그 변수의 영향이 어떤 구조를 갖는가에 대한 주장이며, 변수 조합은 변수들 사이에 상호작용이 있는지와 그 관계가 어떤 형태인지에 대한 주장이다.

그러므로 하나의 Feature Set을 정한다는 것은 성격이 다른 세 종류의 주장을 한꺼번에 정하는 일이며, 세 주장 모두가 각각 데이터로 검증되어야 할 대상이 된다.
...
네 조건 가운데 교차항을 추가한 조건은 나머지와 성격이 다르다. 커널은 선형 그대로 두고, 입력에서 두 변수를 꺼내 곱한 열 하나를 덧붙이는 변환 함수를 만들어 분류기 앞에 파이프라인으로 결합했을 뿐이다.

즉 커널을 고르지 않고 Feature를 하나 만들어 넣는 것만으로도 같은 가설 교체가 일어난다. 커널은 이 교체를 일반화하고 고차원 계산을 우회하게 해 주는 장치이지, 가설을 바꾸는 유일한 통로는 아니다.
...
여기서 확인할 것은 경계의 모양보다 γ가 맡은 역할이다. γ가 커지면 고차항이 더 많이 쓰이므로 결정 함수가 그만큼 복잡해진다. 따라서 γ를 정한다는 것은 데이터를 어느 정도의 복잡도로 다룰 것인가를 정하는 일이며, 어떤 커널을 쓸지 고르는 것과 마찬가지로 그 자체가 하나의 가설 선택이다. 
...
세 Set을 비교하기 전에 원본 변수와 가격의 관계를 먼저 살펴 두면 각 Set이 담은 가설이 어디에서 나왔는지가 드러난다. 세 변수와 가격을 짝지어 놓고, 여기에 두 변수를 곱한 값과 가격의 관계까지 함께 본다.

면적과 가격 사이에는 선형 관계가 보이지만 변동이 상당히 크다. 위치지수와 가격은 관련이 뚜렷하고, 직선이 아니라 완만하게 휘어 오르는 지수 형태에 가깝다. 방수와 가격은 변동이 워낙 커서 거의 잡음처럼 보인다.

면적과 위치지수를 곱한 값을 가격과 놓고 보면 관련성이 뚜렷하게 나타난다. 두 변수가 함께 작용한다는 뜻이다. Engineered Set에 들어간 비선형 항과 상호작용 항은 임의로 고른 것이 아니라 이렇게 읽어 낸 단서를 가설로 옮긴 것이다.
...
Original을 기본 성능으로 놓고 보면, 전처리는 성능을 올리기도 하고 내리기도 한다. 비선형 항과 면적·위치의 상호작용을 더한 Engineered는 기본 성능을 조금 넘어섰고, PCA로 두 축까지 압축한 Reduced는 정보가 손실되어 R²가 크게 내려앉았다. 전처리 선택이 성능의 방향을 정한다.
...
PI는 열을 어떻게 섞느냐에 따라 값이 흔들린다. 그래서 한 번 돌린 값을 그대로 쓰지 않고 30회 반복해 평균을 내며, 표본 표준편차를 반복 횟수의 제곱근으로 나눈 표준오차까지 함께 구해 그 흔들림의 폭을 수치로 남긴다.
...
PI가 R² 하락량이라는 간접적인 척도인 것과 달리, SHAP은 예측값 자체를 각 Feature의 기여도 합으로 분해한다. 그래서 여기 나열된 값은 각 Feature가 예측 가격을 얼마나 움직였는지의 평균 크기이며, area*location만 나머지보다 한 자릿수 큰 값을 갖는다.
...
1위인 area*location은 세 metric에서 모두 같지만, 2위 자리는 location_sq와 location_score가 서로 자리를 바꾼다. 순위표만으로는 보이지 않는 격차의 크기를 막대그래프가 함께 보여 준다. 확인할 지점은 값의 크기가 아니라 metric을 바꾸어도 순위가 유지되는가다.
...
실무에서는 적용 순서가 있다. 모델을 만들기 전에 Mutual Information으로 의미 없는 Feature를 걸러 모델에 넣을 변수를 줄이고, 학습이 끝난 뒤 Permutation Importance로 global 중요도를 확인한 다음, 두 단계 사이에서 순위가 바뀌었는지를 점검한다.
...
이 기대가 어디에서 나왔는지는 수동 작업의 실제 모습을 보면 분명해진다. 변수를 나누어 보고 곱해 보고 로그와 지수를 씌워 보며 후보를 여럿 만든 뒤, 그 가운데 가장 나은 것을 골라 모델을 학습시키는 반복 작업이다. 딥러닝은 이 과정을 모델 안으로 옮겨 층이 스스로 Feature를 만들게 한다.

그래서 층을 충분히 쌓으면 적절한 함수가 만들어지므로 수동 전처리가 필요 없다는 설명이 따라붙는다. 그러나 전처리를 하지 않았다고 해서 가설이 사라진 것은 아니다. 모델이 학습을 통해 가설을 발견하는 것일 뿐이다.
...
세 표현은 분류기 앞단에 놓인 전처리의 층위가 서로 다르다. 픽셀은 전처리를 전혀 거치지 않은 원본이고, HOG는 사람이 정한 규칙으로 한 번 전처리한 결과이며, AE 잠재 표현은 학습을 통해 얻은 것이다. 이 세 층위를 나란히 놓고 비교하는 것이 이 실습의 설계다.
...
세 덴드로그램은 묶이는 짝만 다른 것이 아니라 병합 높이, 곧 클래스 사이의 거리 개념까지 다르다. 한 표현에서 가장 먼저 묶인 3과 5가 다른 표현에서는 멀리 떨어져 있고, 또 다른 표현에서는 5와 8이 묶인 뒤에야 3이 합류한다.
...
여기서 따라 나오는 실무적 경고가 있다. 데이터를 분석해 두 클래스가 서로 가깝다고 판단하는 일은, 그 판단이 어떤 Feature 위에서 얻어진 것인지를 밝히지 않으면 위험하다. 묶이는 짝만이 아니라 클래스 사이의 간격이라는 개념 자체가 표현마다 다르게 잡히기 때문이다.

Soft Label의 재사용도 마찬가지다. 어떤 절차로 클래스 사이의 관계를 찾아 Soft Label을 만들어 두었더라도, 입력 Feature의 전처리나 모델 구조가 바뀌면 그 Soft Label은 그대로 쓸 수 없다. 클래스 사이의 유사도를 처음부터 다시 정의해야 한다.

같은 논리가 사람에게도 적용된다. Feature를 다르게 고른 도메인 전문가는 같은 데이터에서 다른 관계를 본다. 그리고 그가 세운 가설 위에서만 결론이 만들어지므로 그 가설 밖의 새로운 패턴은 발견되기 어려워진다. 모델의 구조 역시 같은 방식으로 작용해, 어떤 연산을 쌓았느냐에 따라 서로 다른 Feature가 만들어진다.
...
대체 Feature Set 비교에는 판정 기준이 따라붙는다. 두 개 이상의 Set에서 결론이 유지되면 그 가설은 안정적이라고 볼 수 있고, 결과가 크게 달라지면 가설 자체의 변동성이 크다는 뜻이므로 그 결론은 믿을 수 없다. 한편 Feature 명세서는 실무에서 가장 자주 건너뛰는 단계다.
...
그래서 모델을 돌려 얻은 결과가 그 자체로 객관적으로 만들어졌다고 보는 것은 착각이다. 대부분의 결과는 이 주체들이 Feature를 어떻게 다룰지 정한 결정에서 나온다. 무엇을 발견했다는 사실을 그대로 믿을 것이 아니라 어떤 가설 위에서 그 구조가 만들어졌는지를 되짚어 검증해야 한다.

공동 가설이라는 규정에는 공동 책임이 따라붙는다. 어느 한 주체의 판단만으로 Feature를 정하면 의도하지 않은 결과가 만들어질 수 있고, 그 결과에 대한 책임 추궁도 함께 따라온다. 그래서 Feature 선택은 책임 주체들의 공동 결정으로 다루어야 한다.
