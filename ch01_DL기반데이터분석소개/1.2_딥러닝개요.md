...
$$[x_1\; x_2\; \cdots\; x_d]\, W + b = [y_1\; y_2\; \cdots\; y_m], \qquad W \in \mathbb{R}^{d\times m}$$

$$f : Y=\sigma(X \cdot W + b), \qquad \sigma = \mathrm{softmax}\;(\text{sum to } 1)$$

- 입력 특징 예: Age, Height, Weight, Blood pressure, …
- 출력 예: Clean, Cancer, Cold, High blood pressure, … (각 클래스의 확률)

입력 특징 $x \in \mathbb{R}^{d}$ 가 가중치 행렬 $W \in \mathbb{R}^{d \times m}$ 와 편향 $b$ 를 거쳐 $y \in \mathbb{R}^{m}$(sum to 1)으로 변환된다. 오른쪽 신경망 도식은 Linear activation을 수행하는 Cell 1~3의 출력이 Softmax를 통해 확률 분포 $y_1, y_2, y_3$ 로 정규화되는 과정을 보여준다.

![다항 로지스틱 회귀의 입출력 구조 — 입력 특징 벡터가 W와 b를 거쳐 sum to 1인 출력 벡터로 변환되고, 신경망 도식에서 Linear activation 뒤에 Softmax가 붙는다](images/fig018.png)

*그림 1-6. 다항 로지스틱 회귀의 입출력 구조*
...
logits(scores) $[2.0,\,1.0,\,0.1]$ 이 Softmax를 통해 확률 $[0.7,\,0.2,\,0.1]$(sum to 1)로 변환된다. 신경망 도식에서는 Cell 1~3의 선형 출력 $\check{y}$ 가 Softmax를 거쳐 예측 확률 분포 $\hat{y}$ 가 된다.

![Softmax 변환 — logits 2.0, 1.0, 0.1이 확률 0.7, 0.2, 0.1로 바뀌고, 신경망의 Cell 출력이 Softmax를 거쳐 확률 분포가 된다](images/fig019.png)

*그림 1-7. Softmax: logits에서 확률 분포로*
...
*그림 1-12. Activation function의 saturation 영역*
